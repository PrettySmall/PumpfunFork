...
export async function updateToken(name, ticker, desc, logo, twitter, telegram, kick, website, mintAddr) {
  const result = await axiosPrivate.post(`/token/update_token`, {
    name, ticker, desc, logo, twitter, telegram, kick, website, mintAddr
  });
  return result.data;
}
...
export async function checkBadWord(text) {
  const result = await axiosPublic.get(`/token/check_bad_word?text=${text}`)
  return result.data
}
...
export async function fetchTrade(mintAddr, lastId) {
  let reqUrl = `/token/fetch_trade?mintAddr=${mintAddr}`
  if(lastId != null && lastId != '') {
    reqUrl += `&lastId=${lastId}`
  }
  const result = await axiosPublic.get(reqUrl)
  return result.data
}
