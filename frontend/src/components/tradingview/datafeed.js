...
import { DATATYPE_PRICE } from "../../engine/consts";
...
const datafeed = (tokenId) => {
  return {
    ...
    getBars: async (
      ...
      try {
        ...
        if(bars && bars.length > 0) {
          let newLatestBar = bars[bars.length - 1];
          if(latestBar == null || newLatestBar.time > latestBar.time) {
            latestBar = newLatestBar;
            const seconds = INTERVAL_SECONDS[convertResolution(resolution)]

            var txTime = (0 + Math.floor(latestBar.time / (seconds * 1000))) * seconds * 1000
            latestBar.time = txTime
            // console.log('datafeed.js->getBars->latestBar:', latestBar)
          }
        }
    ...
    subscribeBars: (
      ...
      const initWebSocket = () => {
        if(moduleState.webSocket) {
          try {
            moduleState.webSocket.close()
          } catch (error) {
            console.log('Chart->datafeed.js->initWebSocket->close_socket->error:', error)
          }
        }
        let ws = new WebSocket(process.env.NEXT_PUBLIC_SOCKET_URL + "?mintAddr=" + tokenId);
        moduleState.webSocket = ws
        ws.onopen = () => {
          window.delta = 0;
          let sendData = {
            action: "subscribe",
        ...
        ws.onmessage = (event) => {
          ...
          if (data.type === DATATYPE_PRICE && transaction && transaction.tokenId == tokenId) {
            const seconds = INTERVAL_SECONDS[convertResolution(resolution)]

            var txTime = (0 + Math.floor(transaction.timestamp / (seconds * 1000))) * seconds * 1000
            ...
            if (latestBar && txTime == latestBar.time) {
              ...
              let close = transaction.price
              let open = latestBar.close
              let low = close
